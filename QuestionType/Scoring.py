...
class Scoring(Question):
    ...
    def __str__(self):
        ...
        result += f"，打分项共{len(self.choices)}个：\n"
        result += "\n"
        for index, choice in enumerate(self.choices):
            # 使用索引获取选项名
            choice_name = self.choice_names[index]
            result += f"{choice_name}. {choice}\n"
        result += "\n"
    ...
    def set_option(self, option):
        ...
        if option == "":
            ...
            self.specified = input("请输入要指定的那个打分项（大写小写均可）：").upper()
            if option == "2":
